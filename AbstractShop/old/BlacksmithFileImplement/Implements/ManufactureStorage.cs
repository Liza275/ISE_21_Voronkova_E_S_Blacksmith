...
namespace BlacksmithFileImplement.Implements
{
    public class ManufactureStorage : IManufactureStorage
    {
        ...
        public ManufactureViewModel GetElement(ManufactureBindingModel model)
        {
            ...
            var product = source.Manufactures
   ...
   : 0;
        ...
        private ManufactureViewModel CreateModel(Manufacture manufacture)
        {
            return new ManufactureViewModel
            {
                ...
                ProductComponents = manufacture.ProductComponents.ToDictionary(recPC => recPC.Key, recPC =>(source.Components.FirstOrDefault(recC => recC.Id ==recPC.Key)?.ComponentName, recPC.Value))
            };
        }
    }
}